...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $conn->query("SELECT * FROM appointments WHERE id = $id");
        ...
        <div class="col-lg-10 p-4" style="max-height: 100vh; overflow-y: auto;">
            ...
            <div class="card">
                ...
                <div class="card-body table-responsive">
                    ...
                    <form action="?action=edit-appointment" method="post">
                        <input type="hidden" name="id" value="<?= $row['id'] ?>">
                        <label class="fw-bold">Parent Name</label>
                        <div class="row my-3">
                            <div class="col-lg-6">
                                <input type="text" name="p_lname" class="form-control " value="<?= isset($_POST['p_lname']) ? $_POST['p_lname'] : $row['p_lname'] ?>">
                                <span style="font-size: 13px;">Last Name*</span>
                                <p class="text-danger"><?= isset($_SESSION['error_p_lname']) ? $_SESSION['error_p_lname'] : '' ?></p>
                            </div>

                            <div class="col-lg-6">
                                <input type="text" name="p_fname" class="form-control " value="<?= isset($_POST['p_fname']) ? $_POST['p_fname'] : $row['p_fname'] ?>">
                                <span style="font-size: 13px;">First Name*</span>
                                <p class="text-danger"><?= isset($_SESSION['error_p_fname']) ? $_SESSION['error_p_fname'] : '' ?></p>
                            </div>

                            <div class="col-lg-12">
                                <input type="text" name="p_mname" class="form-control mt-3" value="<?= isset($_POST['p_mname']) ? $_POST['p_mname'] : $row['p_mname'] ?>">
                        ...
                        <input type="date" class="form-control my-2" name="date" min="<?= date('Y-m-d') ?>" value="<?= isset($_POST['date']) ? $_POST['date'] : $row['date'] ?>">
                        <p class="text-danger"><?= isset($_SESSION['error_date']) ? $_SESSION['error_date'] : '' ?></p>

                        <label class="fw-bold">Email Account</label>
                        <input type="email" class="form-control my-2" name="email" min="<?= date('Y-m-d') ?>" value="<?= isset($_POST['email']) ? $_POST['email'] : $row['email'] ?>">
